...
namespace LibraryServices.FamilyService.Controllers.V1
{
    ...
    public class FamilyController : ApiControllerBase
    {
        ...
        public FamilyController(IMinioClient minioClient, ILogger<FamilyController> logger, IRedisBasketRepository redis,
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<MessageData<PageData<FamilyBasicDTO>>> GetFamiliesPageAsync(string? keyword = null, long? categoryId = null, int pageIndex = 1, int pageSize = 30, string? order = "name")
        {
            var redisKey =
            ...
            _logger.LogInformation("query families by category {category} and keyword {keyword} at page {page} pageSize {pageSize}",
